        ...
        for (let i = 0; i < statements.length; i++) {
            ...
            switch (stmt.type) {
                ...
                case 'choice':
                    {
                        /**
                         * Quando incontriamo una scelta o un'istruzione condizionale all'interno
                         * di una scena, vogliamo raccogliere tutte le possibili scelte
                         * disponibili in questo punto. Le scelte possono essere separate da
                         * blocchi `if` che contengono a loro volta solo scelte al loro interno.
                         * Questo permette di mostrare contemporaneamente scelte condizionali.
                         *
                         * La logica seguente scorre i successivi statements finché trova
                         * scelte (`choice`) o condizioni (`if`) i cui rami consistono
                         * esclusivamente di scelte. Per ogni condizione, valuta il
                         * predicato e raccoglie le scelte dal ramo rilevante.
                         */
                        const collectedChoices = [];
                        let j = i;
                        // Funzione ausiliaria per determinare se un array di
                        // istruzioni contiene solo scelte e, in tal caso,
                        // restituirle.
                        const extractChoices = (stmts) => {
                            const ch = [];
                        ...
                        while (j < statements.length) {
                            ...
                            if (s.type === 'choice') {
                                // Scelta diretta
                                collectedChoices.push(s);
                                j++;
                                continue;
                            } else if (s.type === 'if') {
                                // Valutiamo la condizione per decidere quale ramo eseguire
                                const condResult = evalBool(s.condition);
                                const branchStmts = condResult ? s.thenStmts : s.elseStmts;
                                // Estrarre eventuali scelte dal ramo selezionato
                                const branchChoices = extractChoices(branchStmts);
                                if (branchChoices && branchChoices.length > 0) {
                                    // Il ramo contiene solo scelte: includile
                                    collectedChoices.push(...branchChoices);
                                    j++;
                                    continue;
                                } else if (branchStmts.length === 0) {
                                    // Il ramo selezionato non contiene istruzioni: saltalo e continua
                                    j++;
                                    continue;
                                } else {
                                    // Il blocco condizionale contiene altre istruzioni: interrompe la raccolta
                                    break;
                                }
                            } else {
                                // Un altro tipo di istruzione interrompe la raccolta
                                break;
                            }
                        }
                        // Se non abbiamo raccolto alcuna scelta (caso improbabile),
                        // interrompiamo semplicemente questa gestione speciale
                        if (collectedChoices.length === 0) {
                            break;
                        }
                        // Aggiorna l'indice per saltare tutte le istruzioni di scelta/if appena elaborate
                        i = j - 1;
                        // Visualizza le scelte e attendi la selezione dell'utente
                        const selectedStatements = await handleChoices(collectedChoices);
                        // Esegui le istruzioni associate alla scelta selezionata
                        const res = await runStatements(selectedStatements);
                        if (res === 'goto') {
                            return 'goto';
                        }
                        // Dopo aver eseguito la scelta, prosegui con le istruzioni rimanenti
                        continue;
                    }
                default:
